...
void cargarL1(int vC[20], float vP[20]) 
{
    ...
    for (int i = 0; i < 20; i++) 
    {
        if (i == 0) 
        {
            cout << "Ingrese el codigo de articulo (" << i + 1 << "): "; 
            cin >> C;
        ...
        {
            cout << "\nIngrese el codigo de articulo (" << i + 1 << "): "; 
            cin >> C;
            cout << "Ingrese su precio: $"; cin >> P;
            vC[i] = C;
            vP[i] = P;
...
void Proceso(int vCodigo[20], float vPrecio[20], int vAcu[20], int vMes[12],
             bool& procesoCorrecto) 
{
...
void FPA(int vCodigo[20], int vAcu[20]) 
{
    ...
    for (int i = 0; i < 20; i++) 
    {
        cout << "Codigo de articulo: " << vCodigo[i]
             << " || Cantidad total de ventas: " << vAcu[i] << endl;
    }
...
void FPC(int vCodigo[20], int vAcu[20]) {
    ...
    for (int i = 0; i < 20; i++) 
    {
        if (vAcu[i] > Promedio) 
        {
            cout << "Las ventas del articulo con codigo " << vCodigo[i] << " (" 
                 << vAcu[i] << " ventas) son mayores que el promedio calculado (" 
                 << Promedio << ")." << endl;
        }
    }
}
