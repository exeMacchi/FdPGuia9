...
void Proceso(int vNT[20], float vIKM[20], float vTR[20], float vTRC[100], int vKilometros[100]) {
...
void FPA(int vNT[20], float vTR[20]) {
    for (int i = 0; i < 20; i++) {
        cout << "La tarifa numero " << vNT[i] << " recaudo en total: $" << vTR[i] << endl;
    }
}
...
void FPB(int vCamiones[100], float vTRC[100]) {
    ...
    for (int i = 0; i < 100; i++) {
        cout << "El camion numero " << vCamionesCopia[i] << " recaudo en total: $" << vTRCCopia[i] << endl;
    }
}

void FPC(int vCamiones[100], int vKilometros[100]) {
    int I = minimo(vKilometros);
    cout << "El camion numero " << vCamiones[I] << " viajo la menor distancia (" << vKilometros[I] << " km).";
}
...
void inicializarB(int vCamiones[100], float vTotalRecaudadoCamion[100], int vKilometros[100]) {
    for (int i = 0; i < 100; i++) {
