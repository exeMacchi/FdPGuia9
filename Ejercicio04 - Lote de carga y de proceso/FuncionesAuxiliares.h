...
int buscarIMinPRotas(int D, bool mViajeChofer[31][30], int mPRotas[31][30],
                     int& minPRotas) 
{
    int iCho;
    bool banMin = false;
...
void totalPiezasRotasQuincena(float& totalPiezasRotasQ1, float& totalPiezasRotasQ2,
                              int mPRotas[31][30]) 
{
    for (int i = 0; i < 15; i++) 
    {
...
void inicializarC(float vTPRotasChoferQ1[30], float vTPRotasChoferQ2[30],
                  float vPromedioPROtasQ1[30], float vPromedioPRotasQ2[30]) 
{
    for (int i = 0; i < 30; i++) 
    {
...
void totalPiezasRotasChoferes(float vTPRotasChoferQ1[30], float vTPRotasChoferQ2[30],
                              int mPRotas[31][30]) 
{
    for (int x = 0; x < 30; x++) 
    {
...
void Promedios(float totalPiezasRotasQ1, float totalPiezasRotasQ2,
               float vTPRotasChoferQ1[30], float vTPRotasChoferQ2[30],
               float vPromedioPRotasQ1[30], float vPromedioPRotasQ2[30]) 
{
    for (int x = 0; x < 30; x++) 
    {
