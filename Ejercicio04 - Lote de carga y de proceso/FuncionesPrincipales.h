...
void inicializarAB(bool vViajeDia[31], int mPRotas[31][30], bool mViajeChofer[31][30], int vKilometros[30]) {
    for (int i = 0; i < 31; i++) {
        vViajeDia[i] = false;
        for (int x = 0; x < 30; x++) {
            mPRotas[i][x] = 0;
...
void Proceso(int vCamiones[30], int vChoferes[30], bool vViajeDia[30], int mPRotas[31][30], bool mViajeChofer[31][30],
             int vKilometros[30]) {
...
void FPA(int vChoferes[30], bool vViajeDia[31], int mPRotas[31][30], bool mViajeChofer[31][30]) {
    int iCho, minPRotas;

    for (int i = 0; i < 31; i++) {
        if (vViajeDia[i]) {
            iCho = buscarIMinPRotas(i, mViajeChofer, mPRotas, minPRotas);
            cout << "El dia " << i + 1 << ", el chofer " << vChoferes[iCho] << " rompio el menor numero de piezas (" 
                 << minPRotas << " piezas)." << endl;
        }
        else {
...
void FPB(int vCamiones[30], int vChoferes[30], int vKilometros[30]) {
    ...
    if (maxKM != 0) {
        cout << "\nEl camion numero " << vCamiones[I] << " realizo la mayor cantidad de kilometros (" 
             << maxKM << " kilometros), y fue conducido por el chofer numero " << vChoferes[I] << "." << endl;
    }
    else {
...
void FPC(int vChoferes[30], int mPRotas[31][30]) {
    ...
    inicializarC(vTPRotasChoferQ1, vTPRotasChoferQ2, vPromedioPRotasQ1, vPromedioPRotasQ2);
    totalPiezasRotasChoferes(vTPRotasChoferQ1, vTPRotasChoferQ2, mPRotas);
    Promedios(totalPiezasRotasQ1, totalPiezasRotasQ2, vTPRotasChoferQ1, vTPRotasChoferQ2, vPromedioPRotasQ1, 
              vPromedioPRotasQ2);

    for (int i = 0; i < 30; i++) {
        if (i < 9) {
            cout.precision(2);
            cout << vChoferes[i] << "  ||  " << vPromedioPRotasQ1[i] << "         ||  " << vPromedioPRotasQ2[i] << endl;
        }
        else {
            cout.precision(2);
            cout << vChoferes[i] << " ||  " << vPromedioPRotasQ1[i] << "         ||  " << vPromedioPRotasQ2[i] << endl;
